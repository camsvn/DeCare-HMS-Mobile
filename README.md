...
to view it full screen, where its description can be fixed and the photo
dropped. The whole set uploads together (or is queued when the server is
unreachable) with those inherited descriptions as its narrations. Those recent
labels are kept per device and offered across patients and sign-ins, so they are
